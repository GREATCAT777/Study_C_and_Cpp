/*#include<stdio.h> ///모양 다른 먹이를 먹었을때 그모양을 몸으로 사용하는 게임
#include<stdlib.h>
#include<windows.h>
#include<conio.h>
#define DIR_LEFT 0
...
void RenderFood(int a) {
	if (a = 1) {
		Map[food1.pos.X][food1.pos.Y] = DRAW_FOOD1;
	}
	if (a = 0) {
	Map[food2.pos.X][food2.pos.Y] = DRAW_FOOD2;
	}
}
...
void Drawmap() {
	...
	for (j = 0; j < 20; j++) {
		for (i = 0; i < 30; i++) {
			...
			case DRAW_FOOD1: printf("＠"); break;
			case DRAW_FOOD2: printf("#"); break;
			}
			//printf("%d", Map[i][j]);
		}
		puts("");
...
void GenFood1() {
	...
	for (i = 1; i < 29; i++) {
		...
		food1.pos.X = arrPos[r].X;
		food1.pos.Y = arrPos[r].Y;
	}
}
...
void GenFood2() {
	...
	for (i = 1; i < 29; i++) {
		...
		food2.pos.X = arrPos[r].X;
		food2.pos.Y = arrPos[r].Y;
	}
}
...
void CheckCollision() {
	...
	while (curNode != NULL) {
		...
		else if (x == food1.pos.X && y == food1.pos.Y) {
			AppendSnake();
